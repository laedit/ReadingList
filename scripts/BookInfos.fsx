...
let private getInfosFromHtml (book : BookInfo) =
    ...
    let searchRawResult = Http.RequestString(fnacSearchUrl + book.Isbn,
                            headers = [ "Referer", "https://fnac.com"; "Accept", "*/*";  "Accept-Language", "fr-FR"; "Accept-Encoding", "*"])
    ...
    let imageUrl =
        (bookPage.Descendants
                (fun node -> node.Name() = "img" && node.AttributeValue("class").Contains("js-ProductVisuals-imagePreview")))
        |> Seq.head
